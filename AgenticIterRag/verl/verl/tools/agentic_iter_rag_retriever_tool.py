...
def _require_config(config: dict, key: str):
    if key not in config or config[key] in (None, ""):
        raise KeyError(f"missing required AgenticIterRag tool config: {key}")
    return config[key]
...
class AgenticIterRagRetrieverTool(BaseTool):
    """AgenticIterRag v1 的 search tool。

    该实现复制自参考链路后在 AIR 内独立维护。它负责调用 recall
    retriever，必要时再调用共享 ranker，并把 top-M 文档格式化给 agent。
    """
    ...
    def __init__(self, config: dict, tool_schema: OpenAIFunctionToolSchema = None):
        ...
        if self.ranker_enabled and self.ranker_backend != "ray_actor":
            raise ValueError(f"unsupported AgenticIterRagRetrieverTool ranker backend: {self.ranker_backend!r}")
        ...
        self.search_timing_jsonl = os.getenv("AGENTIC_ITER_RAG_SEARCH_TIMING_JSONL", "")
    ...
    async def execute(self, instance_id: str, parameters: dict[str, Any], **kwargs):
        ...
        if not query:
            logger.error("No query provided to AgenticIterRagRetrieverTool.execute")
            return (
