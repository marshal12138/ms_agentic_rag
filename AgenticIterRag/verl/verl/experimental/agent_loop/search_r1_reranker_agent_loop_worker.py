...
@ray.remote
class CoSearchAgentLoopWorker(SearchR1RerankerAgentLoopWorkerBase):
    ...
    def __init__(
        ...
        # Check if using trajectory-based reward (e.g., gated_progress_reward_with_trajectory)
        # Get the function name from config - check both 'name' field and direct string value
        score_fn_config = config.get("reranker_score_assign_function")
        score_fn_name = score_fn_config.get("name", "")
        
        self.use_trajectory_reward = "trajectory" in score_fn_name
        logger.info(f"Using trajectory-based reward: {self.use_trajectory_reward} (function: {score_fn_name})")
        
        # Optional: filter out reranker outputs where answer_in_docs=False before training
        self.filter_no_answer_in_docs = config.trainer.get("filter_no_answer_in_docs", False)
        logger.info(f"Filter no-answer-in-docs outputs: {self.filter_no_answer_in_docs}")
    ...
    async def generate_sequences(self, batch: DataProto) -> tuple[DataProto, list, Optional[DataProto]]:
        ...
        print(
            f"[rollout-progress][cosearch-worker:{os.getpid()}] step={global_steps} started "
            f"trajectories=0/{len(batch)} prompts~=0.0/{total_prompts:.1f}",
            flush=True,
        )
        while pending:
            ...
            if done and (done_count == len(batch) or done_count - last_done_logged >= item_interval):
                print(
                    f"[rollout-progress][cosearch-worker:{os.getpid()}] step={global_steps} "
                    f"trajectories={done_count}/{len(batch)} "
                    f"prompts~={done_count / rollout_n:.1f}/{total_prompts:.1f} "
                    f"elapsed_s={now - start:.1f}",
                    flush=True,
                )
                last_done_logged = done_count
            elif not done:
                print(
                    f"[rollout-progress][cosearch-worker:{os.getpid()}] step={global_steps} waiting "
                    f"trajectories={done_count}/{len(batch)} "
                    f"prompts~={done_count / rollout_n:.1f}/{total_prompts:.1f} "
                    f"elapsed_s={now - start:.1f}",
                    flush=True,
    ...
    async def _process_reranker_outputs(
        ...
        # Step 1.5: (Optional) Filter out outputs where answer_in_docs=False
        no_answer_filter_metrics = {}
        if self.filter_no_answer_in_docs:
            before_count = len(flat_outputs)
            flat_outputs = [o for o in flat_outputs if o.extra_fields.get("answer_in_docs", False)]
            filtered_count = before_count - len(flat_outputs)
            no_answer_filter_metrics = {
                "no_answer_filter/before_count": before_count,
                "no_answer_filter/filtered_count": filtered_count,
                "no_answer_filter/remaining_count": len(flat_outputs),
                "no_answer_filter/filter_ratio": filtered_count / before_count if before_count > 0 else 0.0,
            }
            logger.info(f"Filtered {filtered_count}/{before_count} outputs with answer_in_docs=False")
            if not flat_outputs:
                return [], no_answer_filter_metrics
        
        # Step 2: Assign UID and compute final_score
        if self.use_trajectory_reward:
            # Trajectory-based reward: call function directly on all outputs
            self._assign_uid(flat_outputs)  # Assign UID only
            self.score_assign_fn(flat_outputs)  # Compute final_score in-place
        else:
            # Per-output reward: compute score for each output individually
    ...
    def _assign_uid(self, outputs: list[AgentLoopOutput]) -> None:
        """Assign UID only (for trajectory-based reward functions).
    ...
    def _assign_uid_and_final_score(self, outputs: list[AgentLoopOutput]) -> None:
        ...
        for output in outputs:
            ...
            output.extra_fields["reward_extra_info"] = {
                ...
                "num_relevant_in_pool": output.extra_fields.get("num_relevant_in_pool", 0),
            }
    ...
    def _collect_pre_filter_metrics(self, outputs: list[AgentLoopOutput]) -> dict:
        ...
        # Metrics conditioned on answer_in_docs=True
        aid_tool_scores = [o.extra_fields.get("tool_score", 0.0) for o in outputs if o.extra_fields.get("answer_in_docs", False)]
        aid_num_relevant = [o.extra_fields.get("num_relevant_in_pool", 0) for o in outputs if o.extra_fields.get("answer_in_docs", False)]
        
        # Group sizes
        uid_groups = self._group_by_uid(outputs)
        group_sizes = [len(g) for g in uid_groups.values()]
        
        # Gated progress reward specific metrics (if using trajectory-based reward)
        gated_progress_metrics = {}
        if self.use_trajectory_reward:
            # Collect R_hit statistics conditioned on binarized_agent_score
            # Only for ans_in_docs=False cases
            r_hit_agent_0 = []  # R_hit when binarized_agent_score = 0
            r_hit_agent_1 = []  # R_hit when binarized_agent_score = 1
            
            for output in outputs:
                # Check if this output has gated progress info
                if "gated_progress_R_hit" in output.extra_fields:
                    r_hit = output.extra_fields["gated_progress_R_hit"]
                    bin_agent = output.extra_fields["gated_progress_binarized_agent_score"]
                    
                    if bin_agent == 0.0:
                        r_hit_agent_0.append(r_hit)
                    elif bin_agent == 1.0:
                        r_hit_agent_1.append(r_hit)
            
            # Compute averages
            if r_hit_agent_0:
                gated_progress_metrics["gated_progress/avg_R_hit_when_agent_0"] = np.mean(r_hit_agent_0)
                gated_progress_metrics["gated_progress/count_agent_0"] = len(r_hit_agent_0)
            else:
                gated_progress_metrics["gated_progress/avg_R_hit_when_agent_0"] = 0.0
                gated_progress_metrics["gated_progress/count_agent_0"] = 0
            
            if r_hit_agent_1:
                gated_progress_metrics["gated_progress/avg_R_hit_when_agent_1"] = np.mean(r_hit_agent_1)
                gated_progress_metrics["gated_progress/count_agent_1"] = len(r_hit_agent_1)
            else:
                gated_progress_metrics["gated_progress/avg_R_hit_when_agent_1"] = 0.0
                gated_progress_metrics["gated_progress/count_agent_1"] = 0
            
            # Clean up: pop temporary fields to avoid bugs
            for output in outputs:
                output.extra_fields.pop("gated_progress_R_hit", None)
                output.extra_fields.pop("gated_progress_binarized_agent_score", None)
        ...
        return {
            ...
            "pre_filter/ans_in_docs_count": len(aid_tool_scores),
            "pre_filter/ans_in_docs_avg_tool_score": np.mean(aid_tool_scores) if aid_tool_scores else 0.0,
            "pre_filter/ans_in_docs_avg_num_relevant_in_pool": np.mean(aid_num_relevant) if aid_num_relevant else 0.0,
            **gated_progress_metrics,
        }
    ...
    def _filter_groups(
        ...
        """Filter groups: remove groups with all same final_score or size <= 3."""
        filtered_groups = {}
        num_filtered_same_score = 0
        num_filtered_small_size = 0
        
        for uid, group in uid_groups.items():
            # Filter condition 1: group size <= 3 (keep at least 4 samples to have some variance)
            if len(group) <= 3:
