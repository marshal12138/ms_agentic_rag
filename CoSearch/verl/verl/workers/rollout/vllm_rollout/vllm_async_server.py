...
# vLLM 0.13 将 argparse/network 工具拆到了子模块；这里保留旧路径 fallback，
# 避免 AIR reranker GRPO 训练在不同 vLLM 小版本之间直接导入失败。
try:
...
class vLLMHttpServerBase:
    ...
    def __init__(
        ...
        self.nnodes = nnodes
        self.fused_worker_used = fused_worker_used
        self.fused_worker_sub_cls_name = fused_worker_sub_cls_name
        ...
        print("[vLLMHttpServer] max_model_len:", self.config.max_model_len)
    ...
    async def launch_server(self, master_address: str = None, master_port: int = None):
        ...
        # Call get_zeromq_address with fused worker support
        if self.fused_worker_used:
            # For fused worker, use _fuw_execute to call the method
            method_name = f"{self.fused_worker_sub_cls_name}_fwmn_get_zeromq_address"
            zmq_address_refs = [worker._fuw_execute.remote(method_name) for worker in self.workers]
            # Use asyncio.gather to avoid blocking the event loop (Ray ObjectRef is awaitable)
            zmq_addresses = await asyncio.gather(*zmq_address_refs)
        else:
    ...
    async def run_server(self, args: argparse.Namespace):
        ...
        # vLLM 0.13 将 disable_log_requests 改成 enable_log_requests，并新增
        # aggregate_engine_logging；这里按函数签名动态适配，保证 AIR reranker
        # GRPO 训练可以复用同一套 VERL 代码跑不同 vLLM 小版本。
        engine_init_signature = inspect.signature(AsyncLLM.from_vllm_config)
        engine_init_kwargs = {
            "vllm_config": vllm_config,
            "usage_context": usage_context,
            "disable_log_stats": engine_args.disable_log_stats,
        }
        if "enable_log_requests" in engine_init_signature.parameters:
            engine_init_kwargs["enable_log_requests"] = getattr(engine_args, "enable_log_requests", False)
            if "aggregate_engine_logging" in engine_init_signature.parameters:
                engine_init_kwargs["aggregate_engine_logging"] = getattr(engine_args, "aggregate_engine_logging", False)
        else:
            engine_init_kwargs["disable_log_requests"] = getattr(engine_args, "disable_log_requests", True)

        engine_client = AsyncLLM.from_vllm_config(**engine_init_kwargs)

        # Don't keep the dummy data in memory
        await engine_client.reset_mm_cache()

        app = build_app(args)
        # vLLM 0.13 的 init_app_state 去掉了 vllm_config 参数，旧版本仍保留。
        # 用签名判断比版本号判断更稳，避免环境包热更新后继续踩 API 差异。
        init_app_state_signature = inspect.signature(init_app_state)
        if len(init_app_state_signature.parameters) == 3:
...
class vLLMHttpServer(vLLMHttpServerBase):
    ...
    def __init__(
        ...
        super().__init__(config, model_config, rollout_mode, workers, replica_rank, node_rank, gpus_per_node, nnodes, fused_worker_used, fused_worker_sub_cls_name)
...
class vLLMReplica(RolloutReplica):
    def __init__(
        ...
        super().__init__(replica_rank, config, model_config, gpus_per_node, is_reward_model, server_name_prefix)
    ...
    async def launch_servers(self):
        ...
        for node_rank in range(nnodes):
            ...
            # Generate unique server name with prefix to avoid conflicts
            base_name = "reward" if self.is_reward_model else "main"
            prefix = self.server_name_prefix if hasattr(self, 'server_name_prefix') else ""
            name = f"vllm_server_{prefix}{base_name}_{self.replica_rank}_{node_rank}"
            server = self.server_class.options(
                ...
                nnodes=nnodes,
                fused_worker_used=getattr(self, 'worker_group', None) is not None and getattr(self.worker_group, 'fused_worker_used', False),
                fused_worker_sub_cls_name=getattr(self.worker_group, 'sub_cls_name', '') if hasattr(self, 'worker_group') and self.worker_group else '',
            )
