...
def write_llm_io_trace(record: dict[str, Any]) -> None:
    trace_path = os.getenv("COSEARCH_LLM_IO_JSONL", "")
    if not trace_path:
        return
    try:
...
class CoSearchTool(BaseTool):
    ...
    def __init__(self, config: dict, tool_schema: OpenAIFunctionToolSchema = None):
        ...
        self.search_timing_jsonl = os.getenv("COSEARCH_SEARCH_TIMING_JSONL", "")
        ...
        write_llm_io_trace(
            {
                ...
                "output_token_count": len(reranker_output.token_ids),
                "prompt_text": tokenizer.decode(reranker_prompt_ids),
                "output_text": reranked_text,
                "sampling_params": sampling_params,
            }
        )
