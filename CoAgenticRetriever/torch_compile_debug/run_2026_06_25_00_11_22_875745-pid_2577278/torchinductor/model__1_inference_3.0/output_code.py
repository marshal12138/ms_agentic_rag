...
# kernel path: /tmp/torchinductor_root/s4/cs4mabnzocesfccass735i66p6rq5yf5rthmvwdg7xluz5d5vq6m.py
# Topologically Sorted Source Nodes: [logsumexp, pd, mul, sum_1, entropy], Original ATen: [aten._to_copy, aten.logsumexp, npu._npu_dtype_cast, aten._softmax, aten.mul, aten.sum, aten.sub]
# Source node to ATen node mapping:
#   entropy => sub_12
#   logsumexp => abs_1, add_9, amax_1, convert_element_type, eq_8, exp_1, full_default, log, sub_7, sum_2, where
#   mul => mul_7
#   pd => _npu_dtype_cast, amax, div, exp, sub_2, sum_1
#   sum_1 => sum_3
# Graph fragment:
...
#   %amax_1 : [num_users=2] = call_function[target=torch.ops.aten.amax.default](args = (%convert_element_type, [-1], True), kwargs = {})
...
#   %full_default : [num_users=1] = call_function[target=torch.ops.aten.full.default](args = ([], 0.0), kwargs = {dtype: torch.float32, layout: torch.strided, device: npu:1, pin_memory: False})
#   %where : [num_users=2] = call_function[target=torch.ops.aten.where.self](args = (%eq_8, %full_default, %amax_1), kwargs = {})
#   %sub_7 : [num_users=1] = call_function[target=torch.ops.aten.sub.Tensor](args = (%convert_element_type, %where), kwargs = {})
...
#   %amax : [num_users=1] = call_function[target=torch.ops.aten.amax.default](args = (%_npu_dtype_cast, [-1], True), kwargs = {})
#   %sub_2 : [num_users=1] = call_function[target=torch.ops.aten.sub.Tensor](args = (%_npu_dtype_cast, %amax), kwargs = {})
...
triton_unk_fused__npu_dtype_cast__softmax__to_copy_logsumexp_mul_sub_sum_0 = async_compile.triton('triton_unk_fused__npu_dtype_cast__softmax__to_copy_logsumexp_mul_sub_sum_0', '''
import triton
import triton.language as tl
...
@npu_triton_heuristics.reduction_npu_index(
    size_hints=[8396, 151936],
    reduction_hint=ReductionHint.INNER,
    filename=__file__,
    triton_meta={'signature': {'in_out_ptr1': '*fp32', 'in_ptr0': '*bf16', 'ks0': 'i64', 'x0_numel': 'i32', 'r1_numel': 'i32'}, 'device': NPUDeviceProperties(type='npu', index=1, multi_processor_count=40, cc='Ascend910B3', major=None, regs_per_multiprocessor=None, max_threads_per_multi_processor=None, warp_size=None), 'constants': {}, 'mix_mode': 'aiv'},
    inductor_meta={'grid_type': 'GridNpu', 'autotune_hints': set(), 'kernel_name': 'triton_unk_fused__npu_dtype_cast__softmax__to_copy_logsumexp_mul_sub_sum_0', 'mutated_arg_names': ['in_out_ptr1'], 'backend_hash': '011adc744ec7327e0966fe326f3190ef00ba89928c4faa3bc3769d863889216f', 'split_axis': [0], 'tiling_axis': [0, 1], 'axis_names': ['x0', 'r1'], 'low_dims': {0, 1}, 'numof_reduction_axis': 1, 'split_axis_dtype': torch.float32, 'dual_reduction': False, 'traced_graph_hash': 'TRACED_GRAPH_HASH', 'traced_graph_dir': 'TRACED_GRAPH_DIR', 'store_cubin': False, 'force_disable_caches': False, 'profile_bandwidth_with_do_bench_using_profiling': False}
)
@triton.jit
def triton_unk_fused__npu_dtype_cast__softmax__to_copy_logsumexp_mul_sub_sum_0(in_out_ptr1, in_ptr0, ks0, x0_numel, r1_numel, X0BLOCK : tl.constexpr, X0BLOCK_SUB : tl.constexpr, R1BLOCK_SUB : tl.constexpr):
...
def call(args):
    ...
    triton_unk_fused__npu_dtype_cast__softmax__to_copy_logsumexp_mul_sub_sum_0.run(buf5, arg2_1, s57, 8396, 151936, stream=stream1)
...
def benchmark_compiled_module(times=10, repeat=10):
    ...
    arg0_1 = 8396
    arg1_1 = 151936
    arg2_1 = rand_strided((8396, 151936), (151936, 1), device='npu:1', dtype=torch.bfloat16)
