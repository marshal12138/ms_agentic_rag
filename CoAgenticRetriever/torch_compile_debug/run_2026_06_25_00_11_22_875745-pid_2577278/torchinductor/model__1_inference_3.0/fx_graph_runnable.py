...
os.environ['TORCH_HOME'] = '/data05/cache/torch'
...
class Repro(torch.nn.Module):
    ...
    def forward(self, arg0_1, arg1_1, arg2_1):
        ...
        amax = torch.ops.aten.amax.default(_npu_dtype_cast, [-1], True)
        sub_2 = torch.ops.aten.sub.Tensor(_npu_dtype_cast, amax);  _npu_dtype_cast = amax = None
        ...
        amax_1 = torch.ops.aten.amax.default(convert_element_type, [-1], True)
        abs_1 = torch.ops.aten.abs.default(amax_1)
        eq_8 = torch.ops.aten.eq.Scalar(abs_1, inf);  abs_1 = None
        full_default = torch.ops.aten.full.default([], 0.0, dtype = torch.float32, layout = torch.strided, device = device(type='npu', index=1), pin_memory = False)
        where = torch.ops.aten.where.self(eq_8, full_default, amax_1);  eq_8 = full_default = amax_1 = None
        squeeze = torch.ops.aten.squeeze.dims(where, [-1])
        sub_7 = torch.ops.aten.sub.Tensor(convert_element_type, where);  convert_element_type = where = None
        ...
        mul_7 = torch.ops.aten.mul.Tensor(div, arg2_1);  div = arg2_1 = None
        sum_3 = torch.ops.aten.sum.dim_IntList(mul_7, [-1], dtype = torch.float32);  mul_7 = None
        sub_12 = torch.ops.aten.sub.Tensor(add_9, sum_3);  add_9 = sum_3 = None
        return (sub_12,)
        
def load_args(reader):
    reader.symint(8396)  # arg0_1
    reader.symint(151936)  # arg1_1
    buf0 = reader.storage(None, 2*s57*(s1 - 1) + 2*s57, device=device(type='npu', index=1), dtype_hint=torch.bfloat16)
