...
本目录包含 async-labeling 使用的轨迹选择器。轨迹选择器运行在 rollout 之后、LLM-as-judge 请求构造之前。它的输出是一组 `ToolCallContext` 对象，而不是对比学习样本。
...
```text
async_ranker_strategies/trajectory_selector/best_and_worst_f1.py
```
...
```text
async_ranker_strategies/trajectory_selector/select_all.py
```
...
### 方案 1：编辑 async labeling 策略 YAML

对于 DeepSeek-Flash async-labeling 训练任务，编辑：
...
```bash
bash /data01/ms_wksp/agent_up_to_date/CoSearch_derevitives/tasks/train_tasks/train_CAR_async_labeling_ds_flash_mix_signal.sh
```
...
```bash
COAGENTIC_EXTRA_ARGS='ranker_training.async_labeling.trajectory_selector.type=select_all' \
bash /data01/ms_wksp/agent_up_to_date/CoSearch_derevitives/tasks/train_tasks/train_CAR_async_labeling_ds_flash_mix_signal.sh
```
...
```bash
COAGENTIC_EXTRA_ARGS='ranker_training.async_labeling.trajectory_selector.type=select_all ranker_training.async_labeling.max_sub_query=32' \
bash /data01/ms_wksp/agent_up_to_date/CoSearch_derevitives/tasks/train_tasks/train_CAR_async_labeling_ds_flash_mix_signal.sh
```
...
```text
async_labeling/selector_contexts
async_labeling/selected_tool_calls
