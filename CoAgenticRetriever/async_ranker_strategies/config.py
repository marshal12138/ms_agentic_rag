...
def build_async_signal_builder(config: SampleBuilderConfig):
    ...
    if "signal_builder_type" not in strategy_kwargs:
        raise KeyError("missing required ranker config: ranker_training.async_labeling.sample_builder.strategy_kwargs.signal_builder_type")
    ...
    if signal_type in {"llm_judge_topk", "llm_judge_top1"}:
        if "positive_top_k" not in strategy_kwargs:
            raise KeyError("missing required ranker config: ranker_training.async_labeling.sample_builder.strategy_kwargs.positive_top_k")
        if "label_source" not in strategy_kwargs:
            raise KeyError("missing required ranker config: ranker_training.async_labeling.sample_builder.strategy_kwargs.label_source")
