...
def config_get_bool(config: Any, path: str, default: bool) -> bool:
    current = config
    for key in path.split("."):
        if current is None:
            return default
        try:
            current = current.get(key) if hasattr(current, "get") else getattr(current, key)
        except Exception:
            return default
...
class CoAgenticRetrieverAgentLoop(AgentLoopBase):
    ...
    def init_class(cls, config, tokenizer, processor, **kwargs):
        ...
        cls.system_prompt = tokenizer.apply_chat_template(
            [{}], add_generation_prompt=False, tokenize=True, **cls._chat_template_kwargs()
        )
        print(f"[COAGENTIC RETRIEVER AGENT] inject_tool_schema={cls.inject_tool_schema}")
        ...
        if cls.inject_tool_schema:
            assert cls.tool_schemas, "CoAgenticRetrieverAgentLoop requires tool schemas for Qwen tool formatting."

    @classmethod
    def _chat_template_kwargs(cls) -> dict[str, Any]:
        kwargs = dict(cls.apply_chat_template_kwargs)
        if cls.inject_tool_schema:
            kwargs["tools"] = cls.tool_schemas
        return kwargs
    ...
    async def _call_tool(
        ...
        turn_idx = len(agent_data.tool_call_details)
        tool_call_detail = {
            "tool_call_id": f"{agent_data.extra_fields['uid']}:search:{turn_idx}",
            "trajectory_id": agent_data.extra_fields["uid"],
            "origin_query": agent_data.initial_query,
            "step_index": turn_idx,
            "turn_idx": turn_idx,
            "score_type": "f1",
            "sub_query": res["sub_query"],
            "tool_score": res["tool_score"],
            "answer_in_docs": res["answer_in_docs"],
            "ranker_failed": res["ranker_failed"],
