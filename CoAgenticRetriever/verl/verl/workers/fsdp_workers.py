...
class ActorRolloutRefWorker(Worker, DistProfilerExtension):
    ...
    def __init__(self, config: DictConfig, role: str, **kwargs):
        ...
        # Set NCCL environment variables for multi-node communication
        # This must be done BEFORE any NCCL operations (including init_device_mesh)
        # to ensure NCCL uses IPv4 and the correct network interface
        if "NCCL_SOCKET_FAMILY" not in os.environ:
            os.environ["NCCL_SOCKET_FAMILY"] = "AF_INET"  # Force IPv4
        if "NCCL_IB_DISABLE" not in os.environ:
            os.environ["NCCL_IB_DISABLE"] = "1"  # Disable InfiniBand (not available on GCP A2)
        if "NCCL_NET" not in os.environ:
            os.environ["NCCL_NET"] = "Socket"  # Use socket transport
        # Auto-detect network interface if not set
        if "NCCL_SOCKET_IFNAME" not in os.environ:
            try:
                interfaces = [f for f in os.listdir('/sys/class/net/') 
                              if f not in ('lo', 'docker0') and not f.startswith('veth')]
                if interfaces:
                    os.environ["NCCL_SOCKET_IFNAME"] = interfaces[0]
                    os.environ["GLOO_SOCKET_IFNAME"] = interfaces[0]
            except Exception:
                pass  # Fall back to NCCL auto-detection
        ...
        assert self.role in ["actor", "rollout", "ref", "actor_rollout", "actor_rollout_ref",
                             "reranker_rollout", "reranker_actor_rollout", "reranker_ref"]

        self._is_actor = self.role in ["actor", "actor_rollout", "actor_rollout_ref", "reranker_actor_rollout"]
        self._is_rollout = self.role in ["rollout", "actor_rollout", "actor_rollout_ref", "reranker_rollout", "reranker_actor_rollout"]
        self._is_ref = self.role in ["ref", "actor_rollout_ref", "reranker_ref"]
...
class AsyncActorRolloutRefWorker(ActorRolloutRefWorker):
    ...
    def get_zeromq_address(self):
        # Ensure ZeroMQ loop is started before returning address
        if hasattr(self.rollout, 'ensure_zmq_loop_started'):
            self.rollout.ensure_zmq_loop_started()
