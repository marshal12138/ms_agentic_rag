...
from async_ranker_training.strategies.config import build_async_sample_builder, build_async_signal_builder
...
class RankerAsyncTrainer:
    def __init__(
        ...
        *,
        config: Any,
        async_config: AsyncRankerTrainingConfig,
        async_ranker_labeler: AsyncLabeler,
        ranker_wg,
        replay_buffer,
        collator,
        ...
        self.async_ranker_labeler = async_ranker_labeler
    ...
    def get_metrics(self) -> dict[str, Any]:
        with self._metrics_lock:
            ...
            for key in ("ranker/async_updates", "ranker/async_skipped", "ranker/async_consumed_signals", "ranker/async_built_samples"):
                latest.pop(key, None)
            out.update(latest)
        out.update({f"async_ranker_training/labeler_{k}": v for k, v in self.async_ranker_labeler.get_metrics().items()})
        return out
    ...
    def try_train_once(self, *, wait: bool = False, timeout: float | None = 0.0) -> bool:
        signals = self.async_ranker_labeler.completed.pop_latest(
