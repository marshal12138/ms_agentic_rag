"""Configuration helpers for async ranker training."""
...
@dataclass(slots=True)
class AsyncRankerTrainingConfig:
    enable: bool
    max_sub_query: int
...
def _require_mapping(config: dict[str, Any], path: str) -> dict[str, Any]:
    ...
    for part in path.split("."):
        if not isinstance(cur, dict) or part not in cur:
            raise KeyError(f"missing required async ranker training config: {path}")
        cur = cur[part]
    if not isinstance(cur, dict):
        raise TypeError(f"async ranker training config must be a mapping: {path}")
    return dict(cur)
...
def _require_bool(config: dict[str, Any], path: str) -> bool:
    ...
    if isinstance(value, str):
        ...
        if lowered in {"0", "false", "no", "off"}:
            return False
    raise TypeError(f"async ranker training config must be a bool: {path}")


def load_async_ranker_training_config(config: Any) -> AsyncRankerTrainingConfig:
    plain = _to_plain(config)
    if "ranker_training" not in plain or "async_ranker_training" not in dict(plain["ranker_training"]):
        raise KeyError("missing required async ranker training config: ranker_training.async_ranker_training")
    root = plain["ranker_training"]["async_ranker_training"]
    if root is None:
        raise KeyError("missing required async ranker training config: ranker_training.async_ranker_training")
    root = dict(root)
    ...
    return AsyncRankerTrainingConfig(
...
def validate_prompt_path(path: str, project_root: str | Path | None = None) -> Path:
    ...
    if resolved is None:
        ...
        raise FileNotFoundError(f"async ranker training prompt file not found: {path}; tried: {tried}")
