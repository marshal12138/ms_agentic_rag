"""Best/worst trajectory selection for ranker training."""

from __future__ import annotations

from typing import Any

from ..schemas import RankerTrajectory, ToolCallContext
from .top_f1 import TrajectoryContextParser


class BestAndWorstTrajectorySelector:
    """Select top-k and bottom-n valid trajectories by final F1/reward score."""

    def __init__(self, top_k: int, bottom_n: int, min_final_reward: float):
        self.top_k = max(0, int(top_k))
        self.bottom_n = max(0, int(bottom_n))
        self.min_final_reward = float(min_final_reward)
        self._top_f1_parser = TrajectoryContextParser()
