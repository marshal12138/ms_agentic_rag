...
if [[ "${RUN_MODE}" == "no-ranker" ]]; then
  export ENABLE_ASYNC_RANKER_TRAINING="${ENABLE_ASYNC_RANKER_TRAINING:-0}"
...
export ENABLE_ASYNC_RANKER_TRAINING="${ENABLE_ASYNC_RANKER_TRAINING}"
...
# 默认等待 agent/ranker/recall/judge 全部 GPU，避免和前一个完整 async-ranker-training 实验抢资源。
if [[ "${RUN_MODE}" == "no-ranker" ]]; then
...
# - inject_tool_schema 是 rollout agent 的显式行为开关，默认 false，用于和 CoSearch-aligned eval 对齐。
...
COAGENTIC_EXTRA_ARGS_RESOLVED="${COAGENTIC_EXTRA_ARGS:-${DEFAULT_COAGENTIC_EXTRA_ARGS}} actor_rollout_ref.rollout.agent.inject_tool_schema=${INJECT_TOOL_SCHEMA}"
if [[ "${RUN_MODE}" != "no-ranker" ]]; then
  COAGENTIC_EXTRA_ARGS_RESOLVED+=" ranker_training.async_ranker_training.sample_builder_request_batch=${SAMPLE_BUILDER_REQUEST_BATCH}"
fi
