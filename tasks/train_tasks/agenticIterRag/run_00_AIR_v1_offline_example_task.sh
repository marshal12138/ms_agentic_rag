...
# 本任务是 AgenticIterRag v1 的离线两阶段训练示例入口。
...
#   默认 stage 顺序是 train_agent -> generate_traces -> build_reranker_dataset -> train_llm_reranker -> infer_matrix。
#
# RESOURCE_CONFIG:
...
#   负责 LLM reranker 的基座模型、数据集 manifest、label policy、candidate top-N、positive top-K 和训练超参。
#
# MODEL_CONFIG:
...
#
# OVERLAY_YAML:
#   对应 tasks/train_tasks/agenticIterRag/configs/offline_two_stage_overlay.yaml。
#   负责本训练 task 的实验级覆盖，例如实验名、trace 生成策略、reranker label 策略和 infer matrix 对照组。
...
bash "${ROOT}/scripts/agenticIterRag_v1/01_pipeline_launcher.sh" \
  ...
  --OVERLAY_YAML=tasks/train_tasks/agenticIterRag/configs/offline_two_stage_overlay.yaml \
  "$@"
