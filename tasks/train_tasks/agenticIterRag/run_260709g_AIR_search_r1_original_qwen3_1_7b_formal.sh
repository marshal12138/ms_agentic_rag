...
# The run executes only the SPAD search_policy_rl sub-stage.
...
bash "${ROOT}/scripts/agenticIterRag_v1/01_pipeline_launcher.sh" \
  ...
  --AGENT_TRAINING_CONFIG=search_r1_original \
  --MODEL_CONFIG=qwen3_1_7b \
  --ROLLOUT_CONFIG=air_async_qwen3_1_7b \
  --OVERLAY_YAML=tasks/train_tasks/agenticIterRag/configs/search_r1_original_qwen3_1_7b_formal_overlay.yaml \
  "$@"
