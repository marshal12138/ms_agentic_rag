...
- `ASYNC_RANKER_TRAINING_YAML` 会覆盖 base 里的 async_ranker_training 默认配置。
- `COAGENTIC_EXTRA_ARGS` 会覆盖 YAML 中的同路径参数。
- `train_CAR_async_ranker_training_ds_flash_mix_signal.sh` 就是通过 `COAGENTIC_EXTRA_ARGS` 把 YAML 里的 `sample_builder_request_batch: 1` 覆盖成 3。

## 当前任务脚本总览

| 脚本 | 默认 EXP_NAME | 实验类型 | 核心差异 |
| --- | --- | --- | --- |
| `train_CAR_naive_acce.sh` | `CAR_mem_speed_no_think_v1` | 原始/非 async ranker training 的加速训练配置 | 不启用 `ENABLE_ASYNC_RANKER_TRAINING`，不启动 LLM judge；主要验证 no-thinking、并发和显存配置下的基础训练吞吐 |
| `train_CAR_async_ranker_training_ds_flash.sh` | `CAR_async_ranker_training_ds_flash_v1` | DeepSeek-V4-Flash async ranker training | 启用 LLM-as-judge 异步 ranker training，每次 ranker update 默认消费 1 条 completed judge signal |
| `train_CAR_async_ranker_training_ds_flash_mix_signal.sh` | `CAR_async_ranker_training_ds_flash_mix_signal_b3_v1` | DeepSeek-V4-Flash async ranker training + 多 signal 混合 | 在 async ranker training 基础上，每次 ranker update 默认消费 3 条 completed judge signals，并等待目标 GPU 空闲后启动 |
...
- 不启用 async ranker training，因此不会启动 DeepSeek-V4-Flash judge 服务。
...
与 async ranker training 脚本的主要差异：

- 未设置 `ENABLE_ASYNC_RANKER_TRAINING=1`，底层默认是 0。
- 未设置 `ASYNC_RANKER_TRAINING_YAML`。
- 未设置 `AUTO_START_LLM_JUDGE` / `AUTO_STOP_LLM_JUDGE`。
- ranker 仍走底层默认 contrastive update，但训练信号不是 DeepSeek judge 异步 ranker training信号。
...
```bash
/data01/ms_wksp/agent_up_to_date/CoSearch_derevitives/tasks/train_tasks/train_CAR_async_ranker_training_ds_flash.sh
```

实验目的：

- 启用 async ranker training 框架。
...
```bash
/data01/ms_wksp/agent_up_to_date/CoSearch_derevitives/tasks/train_tasks/train_CAR_async_ranker_training_ds_flash_mix_signal.sh
```

实验目的：

- 在 DeepSeek-V4-Flash async ranker training 基础上，混合多条 completed judge signals 再构造 ranker contrastive samples。
- 降低单条 sub-query / 单条轨迹 judge label 的偶然性。
- 让每次 ranker update 的训练信号覆盖更多 query/trajectory。

与 `train_CAR_async_ranker_training_ds_flash.sh` 共有的默认配置：
...
```bash
EXP_NAME=CAR_async_ranker_training_ds_flash_mix_signal_b3_v1
...
```bash
ranker_training.async_ranker_training.sample_builder_request_batch=${SAMPLE_BUILDER_REQUEST_BATCH}
```

注意：

- `async_ranker_training_deepseek_flash.yaml` 中默认 `sample_builder_request_batch: 1`。
...
```bash
WAIT_FOR_GPU_RELEASE=0 bash tasks/train_tasks/train_CAR_async_ranker_training_ds_flash_mix_signal.sh
```

当前实验假设：

- 相比每次只用 1 条 judge signal，混合 3 条 signals 可以提升 ranker update 的样本多样性。
- 可能降低 label 噪声，但也可能引入更旧的 completed signals，需观察 `async_ranker_training/labeler_completed_buffer_size`、`ranker/async_consumed_signals`、`ranker/loss`、`ranker/acc@1`、下游 eval 指标。

适合用途：

- 与 `train_CAR_async_ranker_training_ds_flash.sh` 做 `sample_builder_request_batch=1 vs 3` 对照。
- 验证多 signal 混合是否改善 ranker 训练稳定性和最终 QA 指标。

## 相关策略 YAML

### `async_ranker_training_deepseek_flash.yaml`

路径：

```bash
/data01/ms_wksp/agent_up_to_date/CoSearch_derevitives/scripts/coagenticRetriever_local/strategies_yaml/async_ranker_training_deepseek_flash.yaml
```

作用：

- async ranker training 主配置。
- 设置 `ranker_training.signal_source=async_ranker_training`。
...
ranker_training.async_ranker_training.max_sub_query: 10
ranker_training.async_ranker_training.sample_builder_request_batch: 1
ranker_training.async_ranker_training.trajectory_selector.type: best_and_worst_f1
ranker_training.async_ranker_training.trajectory_selector.top_k: 1
ranker_training.async_ranker_training.trajectory_selector.bottom_n: 2
ranker_training.async_ranker_training.stages[0].type: llm_as_judge
ranker_training.async_ranker_training.stages[0].model: DeepSeek-V4-Flash
ranker_training.async_ranker_training.stages[0].score_schema: ranked_ids_top50
ranker_training.async_ranker_training.sample_builder.type: random_negative_repeat
ranker_training.async_ranker_training.sample_builder.strategy_kwargs.positive_top_k: 5
```

覆盖关系：

- 会覆盖 base async_ranker_training 配置。
...
- 对非 async ranker training 的 ranker pseudo label 构造路径做采样策略改动。
- 与 async ranker training 中的 `ranker_training.async_ranker_training.trajectory_selector` 区分：二者路径不同，一个是普通 ranker training selector，一个是 async ranker training 内部 selector。
