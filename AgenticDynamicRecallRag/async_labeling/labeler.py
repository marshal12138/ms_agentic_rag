"""AsyncLabeler facade and worker runtime."""
...
from .logging import AsyncLabelingLogger
...
class AsyncLabeler:
    def __init__(self, config: AsyncLabelingConfig, *, project_root: str | None = None, log_dir: str | None = None):
        self.config = config
        ...
        self.logger = AsyncLabelingLogger(log_dir) if log_dir else None
        stage_cfg = next((dict(stage) for stage in config.stages if stage.get("type") == "llm_as_judge"), {})
    ...
    def start(self) -> None:
        ...
        for idx in range(max(1, int(self.config.num_workers))):
            thread = threading.Thread(target=self._worker_loop, name=f"async-labeler-{idx}", daemon=True)
            thread.start()
