...
class LLMJudgeRank50Stage:
    ...
    def __init__(self, config: dict[str, Any] | None = None, *, project_root: str | Path | None = None):
        self.config = dict(config or {})
        prompt_cfg = stage_prompt_config(self.config)
        self.endpoint = str(self.config.get("endpoint") or "http://127.0.0.1:8067/v1/chat/completions")
        self.model = str(self.config.get("model") or "DeepSeek-V4-Flash")
        self.temperature = float(self.config.get("temperature", 0.0))
        self.max_tokens = int(self.config.get("max_tokens", 1024))
        self.request_timeout_seconds = float(self.config.get("request_timeout_seconds", 600))
        self.max_retries = int(self.config.get("max_retries", 2))
        self.prompt_version = str(prompt_cfg.get("version") or "")
        self.output_mode = self._normalize_output_mode(str(prompt_cfg.get("output_mode") or "no_think"))
        ...
        self.prompt = MarkdownSystemUserPrompt(
            ...
            max_chunk_chars=int(prompt_cfg.get("max_chunk_chars", 512)),
        )
