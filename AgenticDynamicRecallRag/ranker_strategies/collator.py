...
class RankerContrastiveCollator:
    def __init__(self, tokenizer, max_query_length: int = 192, max_doc_length: int = 256):
    ...
    def __call__(self, samples: list[ContrastiveSample]) -> DataProto:
        ...
        query_texts = [sample.query_input for sample in samples]
        ...
        for group in doc_groups:
            for doc in group:
                prefix = f"{doc.title}\n" if doc.title else ""
                doc_texts.append(prefix + doc.text)
