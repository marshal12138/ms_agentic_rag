...
本文梳理 CoAgenticRetriever 新增 async labeling 训练策略的完整运行链路。重点是把训练入口、服务启动、Hydra 配置注入、rollout 后的 LLM judge 请求、ranker contrastive 更新、checkpoint 和日志产物串起来，便于后续复现实验和排查问题。

当前说明对应 2026-06-16 的代码状态。重点路径：

- 任务入口：`tasks/train_tasks/train_CAR_async_labeling_ds_flash.sh`
...
- async labeling 框架：`CoAgenticRetriever/async_labeling`
- async labeling 策略 YAML：`scripts/coagenticRetriever_local/strategies_yaml/async_labeling_deepseek_flash.yaml`
- LLM judge 服务 YAML：`CoAgenticRetriever/async_labeling/configs/llm_judge_vllm_deepseek_flash_gpu06_07.yaml`
- LLM judge 启动脚本：`CoAgenticRetriever/scripts/launch_llm_as_judge.sh`
- 默认 prompt：`CoAgenticRetriever/async_labeling/prompts/llm_judge_rank50_v1.md`

## 1. 总体链路

async labeling 不是替换 agent GRPO 训练，而是替换 ranker contrastive 样本信号构造方式。
...
这个脚本是 async labeling 训练的推荐入口。它复用 01 主训练脚本，但预设 async labeling 相关环境变量。
...
### 3.1 validate_async_labeling_config

当 `ENABLE_ASYNC_LABELING=1` 时，脚本会检查：

1. `ASYNC_LABELING_YAML` 必须存在。
2. `LLM_JUDGE_SERVICE_CONFIG` 必须存在。
3. async labeling YAML 可以被转换为 Hydra dotlist。
...
8. 创建 `<run-log-dir>/async_labeling` 和 `<run-log-dir>/async_labeling/judge_server`。

这一步的目标是让配置问题在训练前失败，而不是等到 rollout 后才失败。

### 3.2 ensure_llm_judge_service

当 `ENABLE_ASYNC_LABELING=1` 时，01 会先检查 LLM judge endpoint：
...
```text
log/train_logs/coAgenticRetriever/<run-name>/async_labeling/judge_server/
...
因此如果用户在 `COAGENTIC_EXTRA_ARGS` 中覆盖了 async labeling 字段，会覆盖策略 YAML。

## 6. 训练侧 async labeling 策略 YAML
...
- `signal_source: async_labeling`：让 trainer 进入 async ranker 信号链路。
- `enable: true`：开启 async labeler，并让 ranker 从 completed buffer 构造训练样本。
...
if ranker_train_enabled:
  ...
  if async_labeling_enabled:
    _submit_async_labeling_requests(main_batch)
...
```text
CoAgenticRetriever/async_labeling/labeler.py
```

训练初始化时：

```text
AsyncLabeler(config, log_dir=<run-log-dir>/async_labeling)
AsyncLabeler.start()
```
...
```text
CoAgenticRetriever/async_labeling/buffer.py
```
...
```text
CoAgenticRetriever/async_labeling/ranker_async_trainer.py
```
...
```text
CoAgenticRetriever/async_ranker_strategies/signal_builder/llm_judge_topk.py
```
...
```text
CoAgenticRetriever/async_ranker_strategies/sample_builder/random_negative_repeat.py
```
...
async_labeler/completed_buffer_dropped_count
```

正常信号：

- `ranker/trace_ranked_docs = ranker/trace_enriched_tool_calls * 50`。
- `async_labeling/invalid_requests` 为 0 或接近 0。
- `async_labeling/accepted_requests` 接近 `max_sub_query`。
- `async_labeler/completed_count` 随 step 增长。
...
async labeling 的目标是给当前 trainable ranker 的排序结果提供外部监督信号。因此输入必须是 ranker 对 recall top50 重排后的 top50。recall top50 是候选池，不是当前 ranker 排序结果。
...
1. `DRY_RUN=1` 是否通过。
2. `ASYNC_LABELING_YAML` 是否指向正确策略 YAML。
...
2. `async_labeling/invalid_requests` 是否接近 0。
3. `async_labeler/completed_count` 是否增长。
4. `async_labeler/failed_count` 是否可接受。
5. `ranker/local_update_step` 是否在 completed signal 能构造有效样本后增长。

训练后：

1. `async_labeling/completed_signals.jsonl` 是否存在。
2. `async_labeling/failures.jsonl` 的失败原因是否符合预期。
