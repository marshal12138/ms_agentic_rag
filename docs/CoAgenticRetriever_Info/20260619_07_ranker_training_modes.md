...
- `created_global_step` / `max_glb_step_lag`：async labeling 请求产生时的主训练 step，用于控制过旧 label signal 是否还可用。

## 1. 结论

当前不是简单只有 `background_ranker_thread: true/false` 两种总模式，而是先分为两条 ranker 数据来源路径：

1. 非 async labeling 的同步 contrastive 路径。
2. async labeling 路径。

在 async labeling 路径里，再由 `background_ranker_thread` 分成两种 ranker update 执行方式：
...
- `trainer.ranker_steps_per_global_step` 只控制非 async labeling 的同步 contrastive 路径。
- 在 async labeling 路径中，`background_ranker_thread: false` 时，ranker 不是由 `trainer.ranker_steps_per_global_step` 控制，而是由 `ranker_training.async_labeling.ranker_updates_per_global_step` 控制每个 `training/global_step` 最多尝试多少次前台 async ranker update。
- 在 async labeling 路径中，`background_ranker_thread: true` 时，主循环不再主动触发 ranker update；ranker 的 `local_update_step` 不再被 global step 的调用次数直接限制。
...
## 3. 模式 A：非 async labeling 同步 contrastive

### 3.1 语义

该模式不走 LLM judge async labeling。主训练每个 global step 完成 rollout 后，从当前 rollout 的 search tool traces 中构造 ranker contrastive 样本，然后在主训练循环内执行 ranker update。
...
具体 `signal_source` 名称以当前策略实现为准；关键是不能满足 async labeling 启用条件：
...
在非 async labeling 路径里，trainer 会执行类似逻辑：
...
## 4. 模式 B：async labeling + 前台 opportunistic ranker update
...
## 5. 模式 C：async labeling + 后台 ranker 训练线程
...
- async labeling 请求也由 global step 提交。
...
- `num_workers`：async labeler 并发 judge 请求 worker 数。
...
如果目标是不使用 LLM judge async labeling，而是按每个 global step 固定更新 ranker，则使用非 async labeling 同步 contrastive：
...
ranker_training.async_labeling.enable
ranker_training.async_labeling.background_ranker_thread
ranker_training.async_labeling.ranker_updates_per_global_step
...
- 出现 `ranker/steps_per_global_step`：通常是非 async labeling 同步 contrastive 路径。
- `ranker/async_background_thread: 0` 且 `ranker/async_attempted_update > 0`：async labeling 前台 opportunistic update。
- `ranker/async_background_thread: 1` 且 `ranker/async_attempted_update: 0`：async labeling 后台 ranker 线程。
