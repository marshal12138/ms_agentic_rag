...
训练入口：`tasks/train_tasks/train_CAR_async_ranker_training_ds_flash.sh`

本文档记录 CoAgenticRetriever 异步排序标签框架的实现工作。重点不是复述设计计划，而是说明当前代码已经如何落地、关键入口在哪里、哪些语义不能改错，以及后续接手人员需要避开的实现坑。

## 一句话状态

当前已经完成 async ranker training 训练链路的代码实现，并用真实 GPU 跑通过 10 step smoke：
...
8. 主训练日志下新增 `async_ranker_training/` 子目录，专门记录 async ranker training 的观察日志。
...
```text
CoAgenticRetriever/async_ranker_training/
  __init__.py
  schemas.py
  config.py
  request_builder.py
...
定义 async ranker training 独立数据结构，不依赖 VERL runtime：
...
1. `_init_ranker_components()` 中初始化原有 ranker 组件后，读取 `ranker_training.async_ranker_training` 配置。
2. 当 `ranker_training.signal_source == "async_ranker_training"` 且 `enable=true` 时：
   - 创建 `AsyncLabeler`
...
4. `_enrich_tool_calls_with_ranker()` 会对每个 tool call 写入：
   ...
   - `rank_top5_docs`
   - `ranked_passages`
5. async 模式下调用 `_submit_async_ranker_training_requests(main_batch)`，只提交请求，不等待结果。
...
background:
  async ranker training labeler worker 写 completed buffer
...
底层脚本默认 async ranker training 是关闭的：
...
因此正式使用 async ranker training 时，优先使用任务脚本 `train_CAR_async_ranker_training_ds_flash.sh`，不要直接裸跑底层脚本后忘记传环境变量。
...
模型路径、GPU、端口、`--max-model-len` 等 vLLM 启动参数都写在服务侧 YAML，不写在训练侧 async ranker training YAML。
...
```yaml
prompt:
  path: CoAgenticRetriever/async_ranker_training/prompts/your_prompt.md
...
async ranker training 的观察日志不直接耦合现有训练 logger，但目录归属于当前 run：

```text
log/train_logs/coAgenticRetriever/<run-name>/async_ranker_training/
```

例如 10 step smoke：

```text
log/train_logs/coAgenticRetriever/260615-2342-async-ds-flash-10step-CAR_async_ranker_training_ds_flash_gpu10/async_ranker_training/
  requests.jsonl
...
```text
requests.jsonl:
  被提交给 async ranker training labeler 的请求摘要。
...
```bash
DRY_RUN=1 bash /data01/ms_wksp/agent_up_to_date/CoSearch_derevitives/tasks/train_tasks/train_CAR_async_ranker_training_ds_flash.sh
```
...
默认 `max_sub_query=10`。它不是每个 trajectory 数量，而是每个 global step 最多提交给 async ranker training labeler 的 search tool call 数。
...
async ranker training 日志不要写到全局散落目录。默认应在：
...
3. `async_ranker_training/invalid_requests` 是否为 0 或接近 0。
...
7. `async_ranker_training/labeler_completed_buffer_size` 是否长期为 0。
8. `async_ranker_training/labeler_request_queue_size` 是否长期堆满。
9. `AUTO_STOP_LLM_JUDGE` 是否会误停别人共用的 judge 服务。
10. prompt 是否从 `CoAgenticRetriever/async_ranker_training/prompts/` 读取，而不是代码硬编码。
