#!/usr/bin/env python3
"""AgenticIterRag v1 的 vLLM-only 推理引擎。

This infer engine intentionally avoids VERL model loading and checkpoint resume.
It keeps the AgenticIterRag inference path explicit:
...
class InferArgs:
    ...
    agent_retry_backoff: float
    agent_http_force_close: bool
    max_retries: int
    retry_delay: float
    retry_backoff: float
    fail_on_error: bool
...
def default_tool_schema_for_class(class_name: str) -> dict[str, Any]:
    if class_name != "verl.tools.agentic_iter_rag_retriever_tool.AgenticIterRagRetrieverTool":
        raise ValueError(
            "tool_config has no explicit tool_schema; only AgenticIterRagRetrieverTool has a local default "
            f"schema in this infer engine, got class_name={class_name!r}"
        )
    return {
        "type": "function",
        "function": {
...
async def complete(
    ...
    for attempt in range(1, attempts + 1):
        ...
        try:
            data = await post_json(session, url, payload, timeout)
            append_jsonl(
                timing_jsonl,
                {
                    "ts": time.time(),
            ...
            if attempt < attempts:
                ...
                delay *= backoff
    raise RuntimeError(f"agent completion failed after {attempts} attempts: {last_error}") from last_error


async def retrieve(session: aiohttp.ClientSession, args: InferArgs, query: str) -> list[dict[str, Any]]:
    last_error: Exception | None = None
...
async def infer_one(
    ...
    for _turn in range(args.max_assistant_turns):
        ...
        assistant_text = await complete(
            ...
            retry_delay=args.agent_retry_delay,
            retry_backoff=args.agent_retry_backoff,
            timing_jsonl=args.agent_timing_jsonl,
            index=idx,
            turn=_turn + 1,
        )
...
async def run_ranker_only(args: InferArgs, ranker: Ranker) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    df = pd.read_parquet(args.data_path)
    if args.max_infer_num >= 0:
        df = df.head(args.max_infer_num)
...
def write_outputs(args: InferArgs, metrics: list[dict[str, Any]], traces: list[dict[str, Any]], summary: dict[str, Any]) -> None:
...
def write_progress_outputs(args: InferArgs, metrics: list[dict[str, Any]], traces: list[dict[str, Any]]) -> None:
    """按已完成前缀增量落盘，避免长任务异常时丢失全部轨迹。"""

    partial_summary = build_summary(metrics)
    write_outputs(args, metrics, traces, partial_summary)


def write_report(args: InferArgs, summary: dict[str, Any], metrics: list[dict[str, Any]], elapsed_s: float) -> None:
    ...
    lines = [
        ...
        "",
        f"- Infer task: `{args.infer_task_name}`",
        ...
        f"- Flush every N: `{args.flush_every_n}`",
...
def make_ranker(args: InferArgs) -> Ranker | None:
    if args.run_mode == "no-ranker":
        return None
...
async def run_infer(args: InferArgs) -> None:
    ...
    if args.run_mode == "ranker-only":
        ...
        connector = aiohttp.TCPConnector(force_close=args.agent_http_force_close)
        async with aiohttp.ClientSession(connector=connector) as session:
            async def guarded(i: int, row: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
                async with semaphore:
                    sample_start = time.perf_counter()
                    try:
                        return await infer_one(session, args, agent_tokenizer, ranker, row, i)
                    except Exception as exc:
                        return build_failed_result(row, i, exc, time.perf_counter() - sample_start)

            tasks = [asyncio.create_task(guarded(i, row)) for i, row in enumerate(records)]
            results = []
            for done_count, task in enumerate(asyncio.as_completed(tasks), start=1):
                results.append(await task)
                # 每完成 flush_every_n 条就重写当前已完成前缀，避免中断时全部丢失。
                if args.flush_every_n > 0 and done_count % args.flush_every_n == 0:
                    progress_metrics = [item[0] for item in results]
                    progress_traces = [item[1] for item in results]
                    write_progress_outputs(args, progress_metrics, progress_traces)
    ...
    if args.fail_on_error and summary.get("failure_count", 0) > 0:
        raise RuntimeError(
            f"infer quality gate failed: failure_count={summary['failure_count']} "
            f"status_counts={summary.get('status_counts')}"
        )


def parse_infer_args(ns: argparse.Namespace) -> InferArgs:
    agent_model = None
    ...
    return InferArgs(
        ...
        max_infer_num=ns.max_infer_num,
        ...
        infer_task_name=ns.infer_task_name,
        ...
        agent_max_retries=ns.agent_max_retries,
        agent_retry_delay=ns.agent_retry_delay,
        agent_retry_backoff=ns.agent_retry_backoff,
        agent_http_force_close=ns.agent_http_force_close,
        ...
        agent_timing_jsonl=ns.agent_timing_jsonl,
...
def main() -> int:
    ...
    run_parser.add_argument("--max-infer-num", type=int, default=-1)
    ...
    run_parser.add_argument("--agent-max-retries", type=int, default=3)
    run_parser.add_argument("--agent-retry-delay", type=float, default=1.0)
    run_parser.add_argument("--agent-retry-backoff", type=float, default=2.0)
    run_parser.add_argument("--agent-http-force-close", action=argparse.BooleanOptionalAction, default=True)
    ...
    run_parser.add_argument("--fail-on-error", action=argparse.BooleanOptionalAction, default=True)
    ...
    run_parser.add_argument("--agent-timing-jsonl", type=Path, default=None)
