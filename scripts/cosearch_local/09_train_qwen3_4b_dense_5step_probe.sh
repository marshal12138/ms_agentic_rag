...
start_retrievers() {
  ...
  for ((i = 0; i < target_instances; i++)); do
    ...
    setsid env PY="${PY}" PORT="${port}" DEVICE="${RETRIEVER_DEVICE}" FAISS_GPU=0 \
      ...
      bash "${ROOT}/scripts/cosearch_local/02b_start_dense_retriever_server.sh" > "${log}" 2>&1 &
    pgid="$!"
...
start_proxy() {
  ...
  setsid "${PY}" "${ROOT}/scripts/cosearch_local/retrieval_round_robin_proxy.py" \
    --host 127.0.0.1 \
    --port "${PROXY_PORT}" \
...
start_nvidia_smi_sampler() {
  ...
  NVIDIA_SMI_PGID=$!
  echo "nvidia-smi sampling enabled: interval=${NVIDIA_SMI_INTERVAL}s csv=${NVIDIA_SMI_CSV}"
}
...
export TOP_N=50
export TOP_M=5
export MAX_PROMPT_LENGTH=11264
...
cosearch_generate_final_training_reports "${ROOT}" || true
