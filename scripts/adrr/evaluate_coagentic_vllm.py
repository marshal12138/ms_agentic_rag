...
async def evaluate_one(
    ...
    for _turn in range(args.max_assistant_turns):
        ...
        max_generation_tokens = args.max_model_len - len(active_prompt_ids)
        ...
        remaining_response_tokens = max(args.max_response_length - response_token_count, 0)
        response_length_reached = response_token_count + len(assistant_ids) >= args.max_response_length
        assistant_ids_for_state = assistant_ids
        assistant_text_for_state = assistant_text
        if response_length_reached and len(assistant_ids_for_state) > remaining_response_tokens:
            assistant_ids_for_state = assistant_ids_for_state[:remaining_response_tokens]
            assistant_text_for_state = agent_tokenizer.decode(assistant_ids_for_state)
        response_token_count += len(assistant_ids_for_state)
        write_llm_io_trace(
            args.llm_io_jsonl,
            {
                "source": "coagentic_eval",
                ...
                "effective_output_token_count": len(assistant_ids_for_state),
                "prompt_text": prompt_text,
                "output_text": assistant_text,
                "effective_output_text": assistant_text_for_state,
                "sampling_params": {
                    "max_tokens": max_generation_tokens,
                    "temperature": args.temperature,
                    "top_p": args.top_p,
        ...
        prompt_ids.extend(assistant_ids_for_state)
        assistant_texts.append(assistant_text_for_state)
        answer_in_current_turn = extract_answer(assistant_text_for_state)

        if response_length_reached:
            final_answer = answer_in_current_turn
            status = "answered" if final_answer else "response_length_exceeded"
            break
        ...
        tool_payloads, assistant_for_history = parse_tool_calls(assistant_text_for_state)
        if len(tool_payloads) == 1:
            tool_payload = tool_payloads[0]
            assistant_history_ids = truncate_after_first_tool_call_ids(agent_tokenizer, assistant_ids_for_state)
            trim_tokens = len(assistant_ids_for_state) - len(assistant_history_ids)
            if trim_tokens > 0:
                prompt_ids = prompt_ids[:-trim_tokens]
                response_token_count -= trim_tokens
                assistant_for_history = agent_tokenizer.decode(assistant_history_ids)
            ...
            top5_by_call.append(final_docs)
            ...
            if response_token_count + len(tool_response_delta_ids) >= args.max_response_length:
                status = "response_length_exceeded"
                break
            prompt_ids.extend(tool_response_delta_ids)
            response_token_count += len(tool_response_delta_ids)
            user_turns += 1
        ...
        messages.append({"role": "assistant", "content": assistant_text_for_state})
        final_answer = answer_in_current_turn
        if final_answer:
    ...
    trace = {
        ...
        "ranked_top5_chunks": top5_by_call,
