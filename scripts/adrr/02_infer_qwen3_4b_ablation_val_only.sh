...
AGENT_SERVED_MODEL="${AGENT_SERVED_MODEL:-coagentic-agent}"
...
MAX_USER_TURNS="${MAX_USER_TURNS:-2}"
...
LLM_JUDGE_PROMPT_PATH="${LLM_JUDGE_PROMPT_PATH:-${PROJECT_ROOT}/async_labeling/prompts/llm_judge_rank50_v1.md}"
...
start_agent_vllm() {
  ...
  if check_vllm "${AGENT_PORT}"; then
    echo "using existing agent vLLM server: http://127.0.0.1:${AGENT_PORT}"
    return 0
  fi
