...
if [[ "${CANONICAL_CONFIG_MODE:-0}" == "1" ]]; then
  if [[ "$#" -gt 0 ]]; then
    echo "ERROR: canonical config mode does not allow asset runner CLI passthrough: $*" >&2
    exit 2
  fi
  if [[ -z "${CANONICAL_HYDRA_ARGS_FILE:-}" || ! -f "${CANONICAL_HYDRA_ARGS_FILE}" ]]; then
    echo "ERROR: CANONICAL_HYDRA_ARGS_FILE is required in canonical config mode." >&2
    exit 2
  fi
  COAGENTIC_MAIN="${COAGENTIC_MAIN:-${PROJECT_ROOT}/main_coagentic_retriever.py}"
  required_paths=("${COAGENTIC_MAIN}" "${PROJECT_ROOT}/verl" "${CANONICAL_HYDRA_ARGS_FILE}")
...
emit("STATIC_RECALL_FINAL_TOP_N", config.get("recall_final_top_n", ""))
emit("STATIC_SEARCH_TOOL_FINAL_TOP_M", config.get("searchTool_final_top_m", ""))
...
' "${TOOL_CONFIG}")"
  ...
  if [[ -z "${STATIC_RECALL_FINAL_TOP_N}" ]]; then
    echo "ERROR: tool config must explicitly set recall_final_top_n for training." >&2
    echo "       In canonical mode this is generated by 01_train_launcher.sh from Hydra recall_retriever.recall_final_top_n." >&2
    exit 2
  fi
  if [[ -z "${STATIC_SEARCH_TOOL_FINAL_TOP_M}" ]]; then
    echo "ERROR: tool config must explicitly set searchTool_final_top_m for training." >&2
    exit 2
  fi
